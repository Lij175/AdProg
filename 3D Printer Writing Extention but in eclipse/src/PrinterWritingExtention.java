...
public class PrinterWritingExtention {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		if (!fileText) {
			...
			Scanner textScannerForCheckingFontSize = new Scanner(text_txt);
	...
	public static boolean checkIfFontSizeGood(Scanner textScanner, double fontSize) {
		while(textScanner.hasNext()) {
			String newLine = textScanner.nextLine();
			double lineLength = (newLine.length() * ((fontSize * 8) + 2));
			if(lineLength > 220) {
				return false;
			}
		}
	...
	public static void addNewCharacter(character newChar, PrintStream ps) throws FileNotFoundException {
		...
		try {
			gcode = new File("C:\\Users\\" + user + "\\eclipse-workspace\\3D Printer Writing Extention but in eclipse\\gcodes\\characters\\"+ newChar.name + ".gcode");
			gcodeScanner = new Scanner(gcode);
		} catch (Exception FileNotFoundException) {
			gcode = new File("C:\\Users\\" + user + "\\eclipse-workspace\\3D Printer Writing Extention but in eclipse\\gcodes\\characters\\box.gcode");
			gcodeScanner = new Scanner(gcode);
		}
